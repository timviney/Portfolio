...
const TanksGame = () => {
    ...
    const errorBox = error && (
        <div className="rounded-md border border-red-500/40 bg-red-500/10 text-red-300 px-4 py-3 relative mb-4 w-full md:w-1/2">
    ...
    return (
        <section className="w-full min-h-screen py-16 flex flex-col items-center justify-center">
            ...
            <h1 className="font-display text-4xl font-bold tracking-tight mb-8">Tanks Game</h1>
            {errorBox}

            {isLoading && (
                    <div className="mt-4 flex items-center">
                        {loadingSpinner}
                        <span className="ml-2 text-muted">Loading game...</span>
                    </div>
                )}
            <div id="unity-container" className="unity-desktop relative flex flex-col items-center">
                <canvas
                    ...
                    aspect-ratio="16/9"
                    tabIndex="-1"
                    className="rounded-md ring-1 ring-line shadow-shadowOne bg-black max-w-full"
                ></canvas>
                {isLoading && (
