...
import { portfolioSeries, seriesInRange } from "../lib/model";
import { formatDate, gbp, gbpShort } from "../lib/format";
import ChartCard, { NoData, chartAxisProps, chartTooltipStyle } from "./ChartCard";

// Total portfolio value over time (carry-forward across every snapshot date in range).

function PortfolioValueChart({ state, range }) {
  const series = seriesInRange(portfolioSeries(state), range.start, range.end);
